...
- [ ] 방 수정 기능
- [ ] 방 삭제 기능
- [ ] 방 상태 관리 (사용 가능, 점검 중 등)

### 3.5 주문 관리
- [ ] `app/admin/orders/page.tsx` 생성
- [ ] 주문 목록 조회 (Railway API)
- [ ] 주문 상태 업데이트
- [ ] 주문 상세 정보 표시
- [ ] 주문 필터링 (상태별, 날짜별)
...
- [ ] `app/page.tsx` 업데이트 (루트 페이지)
- [ ] `app/home/page.tsx` 처리
  - [ ] 옵션 1: 새 구조로 리다이렉트
  - [ ] 옵션 2: 하위 호환성 유지 (URL 파라미터 지원)
- [ ] `app/guide/page.tsx` 처리
- [ ] `app/market/page.tsx` 처리
- [ ] `app/help/page.tsx` 처리

### 5.2 하위 호환성
- [ ] URL 파라미터로 접근 시 토큰 기반으로 변환
- [ ] 기존 링크 동작 확인
- [ ] 마이그레이션 안내 메시지 (선택사항)
